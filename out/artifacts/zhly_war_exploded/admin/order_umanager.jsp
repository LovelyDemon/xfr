...
<%@page import="com.change.OrderDao"%>
<%@page import="com.change.OrderService"%>
...
if(list==null){
	...
	list = service.listorder("",utype,username, start, end);
}
...
%>
  <head>
	...
	function searchorder(){
		...
		th.action="<%=path%>/OrderAction?action_flag=search";
		th.submit();
	}
	
	function first(){
		
		window.location.href = "<%=path%>/OrderAction?action_flag=search&pageNum=1";
		
	}
	function next(){
		
		window.location.href = "<%=path%>/OrderAction?action_flag=search&pageNum=<%=dividePage.getCurrentPage()+1%>";		
	...
	function end(){
		...
		window.location.href = "<%=path%>/OrderAction?action_flag=search&pageNum=<%=dividePage.getPageCount() %>";
			
	}
	
	function changePage(currentPage){
	
		window.location.href = "<%=path%>/OrderAction?action_flag=search&pageNum="+currentPage;
	...
	function del(){
		...
		th.action="<%=path%>/OrderAction?action_flag=del";
		th.submit();		
	...
	function view(){
		...
		var th = document.form1;
		th.action="<%=path%>/OrderAction?action_flag=view&ordid="+getSelectedValue();
		th.submit();		
...
<script src="js/bootstrap.min.js"></script>
 <div id="page-wrapper">
  	       ...
  	       <h3><b>酒店订单管理</b></h3>
  	         <form class="form-horizontal"  name = "form2" action="" method="post">
								<div class="form-group">
									...
									<button type="button" onclick="searchorder()">查找</button></div>
								</div>
								</form>
	...
	<td width=10% ></td>
          ...
          <th style="width: 35%;text-align:center;">预订房型</th>
          <th style="width: 35%;text-align:center;">入住日期</th>
	...
	<tbody>
        <tr class="active">
         <%
   			if(list!=null && !list.isEmpty()){
   				...
   				<tr align="center">
   				<td width=10%><input type="checkbox" name="ids" value="<%=map.get("ordid") %>"/></td>
   				<td width=10%><%=map.get("honame") %></td>
   				<td width=35%><%=map.get("ordkind") %></td>
   				<td width=35%><%=map.get("orddate") %></td>
   				<td><%=map.get("ordjd") %></td>
   	...
   	<tr>
   		<td>
   			<button type="button" onclick="javascript:del();">取消预订</button>
