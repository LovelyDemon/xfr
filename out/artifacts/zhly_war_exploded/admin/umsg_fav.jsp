...
List<Map<String,Object>> list =(List<Map<String,Object>>) request.getAttribute("listfav");
...
if(list==null){
	...
	list = service.list_fav("", username,start, end);
...
%>
  <head>
	...
	function searchrepl(){
		...
		th.action="<%=path%>/UmsgAction?action_flag=fav_search";
		th.submit();
	}
	
	function first1(){
		
		window.location.href = "<%=path%>/UmsgAction?action_flag=fav_search&pageNum3=1";
		
	}
	function next1(){
		
		window.location.href = "<%=path%>/UmsgAction?action_flag=fav_search&pageNum3=<%=dividePage3.getCurrentPage()+1%>";		
	...
	function end1(){
		...
		window.location.href = "<%=path%>/UmsgAction?action_flag=fav_search&pageNum3=<%=dividePage3.getPageCount() %>";
			
	}
	
	function changePage1(currentPage){
	
		window.location.href = "<%=path%>/UmsgAction?action_flag=fav_search&pageNum3="+currentPage;
	...
	function del1(){
		...
		th.action="<%=path%>/UmsgAction?action_flag=del_fav";
		th.submit();		
	...
	function view1(){
		...
		var th = document.form1;
		th.action="<%=path%>/UmsgAction?action_flag=view_fav&favid="+getSelectedValue();
		th.submit();		
...
<script src="js/bootstrap.min.js"></script>
 <div id="page-wrapper">
	...
	<td width=10%></td>
          ...
          <th style="width: 50%;text-align:center;">收藏时间</th>
          <th style="width: 20%;text-align:center;">查看</th>
        ...
        <tr class="active">
         <%
   			if(list!=null && !list.isEmpty()){
   				...
   				<tr align="center">
   				<td width=10%><input type="checkbox" name="ids" value="<%=map1.get("favid") %>"/></td>
   				<td width=20%><%=map1.get("rename") %></td>
   				<td width=50%><%=map1.get("favtime") %></td>
   				<td width=20%><a href="../RecfAction?action_flag=view&reid=<%=map1.get("reid") %>">查看</a></td>
