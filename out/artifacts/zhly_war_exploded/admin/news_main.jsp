...
<%@page import="com.change.NewsDao"%>
...
List<Map<String,Object>> list =(List<Map<String,Object>>) request.getAttribute("listNews");
// 获取 分页对象
...
if(list==null){
	...
	list = service.listNews("", start, end);
}
...
%>
  <head>
	...
	function searchProduct(){
		...
		th.action="<%=path%>/NewsAction?action_flag=search";
		th.submit();
	}
	
	function first(){
		
		window.location.href = "<%=path%>/NewsAction?action_flag=search&pageNum=1";
		
	}
	function next(){
		
		window.location.href = "<%=path%>/NewsAction?action_flag=search&pageNum=<%=dividePage.getCurrentPage()+1%>";		
	...
	function end(){
		...
		window.location.href = "<%=path%>/NewsAction?action_flag=search&pageNum=<%=dividePage.getPageCount() %>";
			
	}
	
	function changePage(currentPage){
	
		window.location.href = "<%=path%>/NewsAction?action_flag=search&pageNum="+currentPage;
 ...
 <div id="page-wrapper">
  	       ...
  	       <h3><b>删改新闻</b></h3>
  	         <form class="form-horizontal"  name = "form2" action="" method="post">
								<div class="form-group">
									<label for="focusedinput" class="col-sm-2 control-label"><b>新闻名称</b></label>
									<div class="col-sm-8"><input type="text" class="form-control1" name="nename" value="<%= newsName!=null?newsName:"" %>"/>
    ...
    <table class="table">
      <thead>
        <tr>
	<td width=10% ></td>
		  <th style="width: 10%;text-align:center;">新闻编号</th>
          <th style="width: 30%;text-align:center;">新闻标题</th>
          <th style="width: 10%;text-align:center;">新闻作者</th>
          <th style=text-align:center;>新闻摘要</th>
        </tr>
      </thead>
	<tbody>
        <tr class="active">
         <%
   			if(list!=null && !list.isEmpty()){
   				...
   				<tr align="center">
   				<td width=10%><input type="checkbox" name="ids" value="<%=map.get("neid") %>"/></td>
   				<td width=10%><%=map.get("neid") %></td>
   				<td width=30%><%=map.get("nename") %></td>
   				<td width=10%><%=map.get("neactor") %></td>
   				<td><%=map.get("nezy") %></td>
