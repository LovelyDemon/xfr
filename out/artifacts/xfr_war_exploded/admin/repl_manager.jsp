...
%>
  <head>
	...
	function searchProduct(){
		...
		th.action="<%=path%>/RecAction?action_flag=pl_search";
		th.submit();
	}
	
	function first(){
		
		window.location.href = "<%=path%>/RecAction?action_flag=pl_search&pageNum=1";
		
	}
	function next(){
		
		window.location.href = "<%=path%>/RecAction?action_flag=pl_search&pageNum=<%=dividePage.getCurrentPage()+1%>";		
	...
	function end(){
		...
		window.location.href = "<%=path%>/RecAction?action_flag=pl_search&pageNum=<%=dividePage.getPageCount() %>";
			
	}
	
	function changePage(currentPage){
	
		window.location.href = "<%=path%>/RecAction?action_flag=pl_search&pageNum="+currentPage;
	...
	function del(){
		...
		th.action="<%=path%>/RecAction?action_flag=pl_del";
		th.submit();		
	...
	function view(){
		...
		var th = document.form1;
		th.action="<%=path%>/RecAction?action_flag=pl_view&replid="+getSelectedValue();
		th.submit();		
  	       ...
  	       <h3><b>推荐路线评论管理</b></h3>
  	         <form class="form-horizontal"  name = "form2" action="" method="post">
								<div class="form-group">
									...
									<div class="col-sm-8"><input type="text" class="form-control1" name="prname" value="<%= recName!=null?recName:"" %>"/>
        ...
        <tr class="active">
         <%
   			if(list!=null && !list.isEmpty()){
   				...
   				<tr align="center">
   				<td width=10%><input type="checkbox" name="ids" value="<%=map.get("replid") %>"/></td>
   				<td width=10%><%=map.get("replid") %></td>
   				<td width=10%><%=map.get("rename") %></td>
   				<td width=10%><%=map.get("uname") %></td>
   				<td><%=map.get("repltime") %></td>
