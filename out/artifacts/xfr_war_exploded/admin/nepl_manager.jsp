...
%>
  <head>
	...
	function searchProduct(){
		...
		th.action="<%=path%>/NewsAction?action_flag=pl_search";
		th.submit();
	}
	
	function first(){
		
		window.location.href = "<%=path%>/NewsAction?action_flag=pl_search&pageNum=1";
		
	}
	function next(){
		
		window.location.href = "<%=path%>/NewsAction?action_flag=pl_search&pageNum=<%=dividePage.getCurrentPage()+1%>";		
	...
	function end(){
		...
		window.location.href = "<%=path%>/NewsAction?action_flag=pl_search&pageNum=<%=dividePage.getPageCount() %>";
			
	}
	
	function changePage(currentPage){
	
		window.location.href = "<%=path%>/NewsAction?action_flag=pl_search&pageNum="+currentPage;
	...
	function del(){
		...
		th.action="<%=path%>/NewsAction?action_flag=pl_del";
		th.submit();		
	...
	function view(){
		...
		var th = document.form1;
		th.action="<%=path%>/NewsAction?action_flag=pl_view&neplid="+getSelectedValue();
		th.submit();		
        ...
        <tr class="active">
         <%
   			if(list!=null && !list.isEmpty()){
   				...
   				<tr align="center">
   				<td width=10%><input type="checkbox" name="ids" value="<%=map.get("neplid") %>"/></td>
   				<td width=10%><%=map.get("neplid") %></td>
   				<td width=10%><%=map.get("nename") %></td>
   				<td width=10%><%=map.get("uname") %></td>
   				<td><%=map.get("nepltime") %></td>
