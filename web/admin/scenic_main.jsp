...
%>
  <head>
	...
	function searchProduct(){
		...
		th.action="<%=path%>/ScenicAction?action_flag=search";
		th.submit();
	}
	
	function first(){
		
		window.location.href = "<%=path%>/ScenicAction?action_flag=search&pageNum=1";
		
	}
	function next(){
		
		window.location.href = "<%=path%>/ScenicAction?action_flag=search&pageNum=<%=dividePage.getCurrentPage()+1%>";		
	...
	function end(){
		...
		window.location.href = "<%=path%>/ScenicAction?action_flag=search&pageNum=<%=dividePage.getPageCount() %>";
			
	}
	
	function changePage(currentPage){
	
		window.location.href = "<%=path%>/ScenicAction?action_flag=search&pageNum="+currentPage;
	...
	function view(){
		...
		var th = document.form1;
		th.action="<%=path%>/ScenicAction?action_flag=view&scid="+getSelectedValue();
		th.submit();		
 ...
 <div id="page-wrapper">
  	       ...
  	       <h3><b>删改景点</b></h3>
  	         <form class="form-horizontal"  name = "form2" action="" method="post">
								<div class="form-group">
									...
									<div class="col-sm-8"><input type="text" class="form-control1" name="scname" value="<%= scenicName!=null?scenicName:"" %>"/>
    ...
    <table class="table">
      <thead>
        <tr>
	<td width=10% ></td>
		  <th style="width: 10%;text-align:center;">景点编号</th>
          <th style="width: 35%;text-align:center;">景点名称</th>
          <th style="width: 35%;text-align:center;">景点位置</th>
          <th style=text-align:center;>门票价格</th>
        </tr>
      </thead>
	<tbody>
        <tr class="active">
         <%
   			if(list!=null && !list.isEmpty()){
   				...
   				<tr align="center">
   				<td width=10%><input type="checkbox" name="ids" value="<%=map.get("scid") %>"/></td>
   				<td width=10%><%=map.get("scid") %></td>
   				<td width=35%><%=map.get("scname") %></td>
   				<td width=35%><%=map.get("sclocal") %></td>
   				<td><%=map.get("scprice") %>元</td>
