...
<div class="col-md-9 blog-head">	
	<form class="form-horizontal" form name="form1" action="" method="post">
		...
		<%
	     ...
	     <div class="blog-top">
	       <img src="<%=path%>/upload/<%=map.get("repic") %>" class="img-responsive" alt="" style="height: 222px; width: 612px; "/>
	         <h4><a href="RecfAction?action_flag=view&reid=<%=map.get("reid") %>"><%=map.get("rename") %></a></h4>
	         <h5>作者 :<%=map.get("reactor") %> | 发布时间 : <%=map.get("redate") %></h5>
	        <p><%=map.get("rezy") %> </p>
	         <a class="hvr-sweep-to-right more"  href="RecfAction?action_flag=view&reid=<%=map.get("reid") %>">详细</a>
	           <div class="links">
		  		  <ul class="blog-links">
		  			<li><a href="RecfAction?action_flag=like1&reid=<%=map.get("reid") %>"><i class="blog_circle"><%=map.get("relike") %></i><span>赞</span></a></li>
		  			<li><a href="RecfAction?action_flag=fav1&reid=<%=map.get("reid") %>"><i class="glyphicon glyphicon-heart"> </i><span>收藏</span></a></li>
		  		  </ul>
		  	   </div>
